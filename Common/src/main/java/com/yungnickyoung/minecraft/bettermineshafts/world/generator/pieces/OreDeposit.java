...
public class OreDeposit extends BetterMineshaftPiece {
    public enum OreType {
        ...
        REDSTONE(3, Blocks.REDSTONE_ORE.defaultBlockState(), BetterMineshaftsCommon.CONFIG.ores.redstone + IRON.threshold),
    ...
    public OreDeposit(CompoundTag compoundTag) {
        ...
        this.oreType = OreType.valueOf(compoundTag.getIntOr("OreType", 0));
    }
    ...
    public void postProcess(WorldGenLevel world, StructureManager structureManager, ChunkGenerator chunkGenerator, RandomSource randomSource, BoundingBox box, ChunkPos chunkPos, BlockPos blockPos) {
        ...
        // Fill with cobble
        this.chanceReplaceSolid(world, box, randomSource, .9f, 0, 0, 0, LOCAL_X_END, LOCAL_Y_END, LOCAL_Z_END, COBBLE);

        // Ore deposit. Ore is more dense in center than edges
        this.chanceReplaceSolid(world, box, randomSource, .65f, 1, 1, 1, LOCAL_X_END - 1, LOCAL_Y_END - 1, LOCAL_Z_END - 1, ORE_BLOCK);
        this.chanceReplaceSolid(world, box, randomSource, .15f, 0, 0, 0, LOCAL_X_END, LOCAL_Y_END, LOCAL_Z_END, ORE_BLOCK);
    }
}
