...
public class VerticalEntrance extends BetterMineshaftPiece {
    ...
    public void postProcess(WorldGenLevel world, StructureManager structureManager, ChunkGenerator chunkGenerator, RandomSource randomSource, BoundingBox boundingBox, ChunkPos chunkPos, BlockPos blockPos) {
        ...
        if (!this.hasTunnel) {
            ...
            if (BetterMineshaftsCommon.DEBUG_LOG && this.hasTunnel) {
                ...
                BetterMineshaftsCommon.LOGGER.info(String.format("(%d, %d) --- %d / %d  (%f%%)", centerPos.getX(), centerPos.getZ(), BetterMineshaftsCommon.surfaceEntrances.size(), BetterMineshaftsCommon.count.get(), (float) BetterMineshaftsCommon.surfaceEntrances.size() * 100 / BetterMineshaftsCommon.count.get()));
            }
        }
    ...
    private void generateVerticalShaft(WorldGenLevel world, RandomSource randomSource, BoundingBox box) {
        ...
        this.replaceAirOrChains(world, box, SHAFT_LOCAL_XZ_START + 1, 0, SHAFT_LOCAL_XZ_START + 1, SHAFT_LOCAL_XZ_END - 1, 0, SHAFT_LOCAL_XZ_END - 1, config.blockStates.mainBlockState);

        // Ladder
        this.replaceAirOrChains(world, box, SHAFT_LOCAL_XZ_START + 2, 1, SHAFT_LOCAL_XZ_START, SHAFT_LOCAL_XZ_START + 2, localYEnd - 4, SHAFT_LOCAL_XZ_START, config.blockStates.mainBlockState);
        ...
        // Doorway
        this.fill(world, box, SHAFT_LOCAL_XZ_START + 1, 1, SHAFT_LOCAL_XZ_START + 4, SHAFT_LOCAL_XZ_START + 3, 2, SHAFT_LOCAL_XZ_START + 4, config.blockStates.stoneWallBlockState);
        this.fill(world, box, SHAFT_LOCAL_XZ_START + 2, 3, SHAFT_LOCAL_XZ_START + 4, SHAFT_LOCAL_XZ_START + 2, 3, SHAFT_LOCAL_XZ_START + 4, config.blockStates.stoneSlabBlockState);
    ...
    private void generateSurfaceTunnel(WorldGenLevel world, RandomSource randomSource, BoundingBox box) {
        ...
        if (facing.getAxis() == tunnelDirection.getAxis()) {
            // Fill in any air in floor with main block
            this.replaceAirOrChains(world, box, tunnelStartX + 1, tunnelFloorAltitude, tunnelStartZ, tunnelEndX - 1, tunnelFloorAltitude, tunnelEndZ, config.blockStates.mainBlockState);

            // Fill with air
            this.fill(world, box, tunnelStartX + 1, tunnelFloorAltitude + 1, tunnelStartZ, tunnelEndX - 1, tunnelFloorAltitude + 3, tunnelEndZ, AIR);
        } else {
            // Fill in any air in floor with main block
            this.replaceAirOrChains(world, box, tunnelStartX, tunnelFloorAltitude, tunnelStartZ + 1, tunnelEndX, tunnelFloorAltitude, tunnelEndZ - 1, config.blockStates.mainBlockState);
